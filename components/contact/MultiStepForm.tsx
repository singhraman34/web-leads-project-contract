...
// --- Funnel Constants ---
const PROPERTY_TYPES = ["1 BHK", "2 BHK", "3 BHK", "Villa", "Office / Commercial"];
const STYLE_PREFERENCES = ["Modern", "Luxury", "Minimal", "Classic", "Scandinavian"];
const PROPERTY_STATUS = ["New Property", "Under Construction", "Renovation"];
const TIMELINE_OPTIONS = ["Need urgently", "Within 1 month", "1–3 months", "3–6 months", "Just exploring"];
const BUDGET_OPTIONS = ["Under 5L", "5L - 15L", "15L - 50L", "50L+"];
const PRIMARY_SERVICES = ["Interior", "Renovation", "Construction", "Painting", "Turnkey Projects"];
...
function FormContent() {
    ...
    const isStep2Complete = hydrated && data.services.length > 0 && data.propertyType !== "" && data.budget !== "" && data.timeline !== "";
    const isStep3Complete = hydrated && data.stylePreference !== "" && data.propertyStatus !== "";
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // STRICT Check: Only show success if the server responds with 2xx
            if (response.ok) {
                setIsSubmitted(true);
            ...
            >
                ...
                <div className="p-8 md:p-12 relative">
                    <AnimatePresence mode="wait" custom={direction}>
                        {step === 1 && (
                            <motion.div
                                key="funnel-1" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
                            ...
                            >
                                <div className="flex flex-col space-y-8 max-w-2xl mx-auto">
                                    <div className="space-y-4">
                                        ...
                                        <input
                                            type="text" placeholder="John Doe" value={hydrated ? data.name : ""} onChange={(e) => updateData({ name: e.target.value })}
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex items-center w-full bg-slate-50 border border-slate-200 rounded-2xl focus-within:bg-white focus-within:border-slate-900 focus-within:ring-4 focus-within:ring-slate-900/5 transition-all overflow-hidden">
                                            ...
                                            <input
                                                type="tel" placeholder="9876543210" value={hydrated ? data.phone : ""}
                                    ...
                                    <div className="space-y-4">
                                        <label className="flex items-center gap-3 text-sm font-black uppercase tracking-[0.2em] text-slate-400">
                                            <Mail className="w-5 h-5 text-slate-900" /> Email Address (Optional)
                                        </label>
                                        <input
                                            type="email" placeholder="you@example.com" value={hydrated ? data.email : ""} onChange={(e) => updateData({ email: e.target.value })}
                                            className="w-full px-6 py-5 bg-slate-50 border border-slate-200 rounded-2xl font-bold focus:bg-white focus:border-slate-900 focus:ring-4 focus:ring-slate-900/5 transition-all outline-none text-base"
                                        />
                                    </div>
                                    <div className="space-y-4">
                                        <label className="flex items-center gap-3 text-sm font-black uppercase tracking-[0.2em] text-slate-400">
                                            <MessageSquare className="w-5 h-5 text-slate-900" /> Brief Project Description (Optional)
                                        </label>
                                        <textarea
                                            placeholder="Tell us briefly about your project..." value={hydrated ? data.description : ""} onChange={(e) => updateData({ description: e.target.value })}
                                ...
                                <div className="pt-10 border-t border-slate-100">
                                    <Button
                                        onClick={handleBasicSubmit} disabled={!isStep1Complete || isSubmitting}
                        ...
                        {step === 2 && (
                            <motion.div
                                key="step-2" custom={direction} variants={stepVariants} initial="initial" animate="animate" exit="exit"
                                className="space-y-12"
                            >
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-12">
                                    ...
                                    <div className="space-y-6">
                                        <label className="flex items-center gap-3 text-sm font-black uppercase tracking-[0.2em] text-slate-400">
                                            <Layout className="w-4 h-4 text-slate-900" /> Property Type <span className="text-red-500">*</span>
                                        </label>
                                        <div className="grid grid-cols-2 gap-3">
                                            {PROPERTY_TYPES.map(t => (
                                                <button
                                                    key={t} type="button" onClick={() => updateData({ propertyType: t })}
                                    ...
                                    <div className="space-y-6">
                                        <label className="flex items-center gap-3 text-sm font-black uppercase tracking-[0.2em] text-slate-400">
                                            <Building2 className="w-4 h-4 text-slate-900" /> Planned Budget <span className="text-red-500">*</span>
                                        </label>
                                        <div className="grid grid-cols-2 gap-3">
                                            {BUDGET_OPTIONS.map(b => (
                                                <button
                                                    key={b} type="button" onClick={() => updateData({ budget: b })}
                                    ...
                                    <div className="space-y-6">
                                        <label className="flex items-center gap-3 text-sm font-black uppercase tracking-[0.2em] text-slate-400">
                                            <Calendar className="w-4 h-4 text-slate-900" /> Timeline <span className="text-red-500">*</span>
                                        </label>
                                        <div className="grid grid-cols-2 gap-3">
                                            {TIMELINE_OPTIONS.map(t => (
                                                <button
                                                    key={t} type="button" onClick={() => updateData({ timeline: t })}
                                ...
                                <div className="pt-10 border-t border-slate-100 flex justify-between items-center">
                                    <button onClick={handleBack} className="h-16 px-10 rounded-2xl bg-white border border-slate-200 text-slate-600 font-bold uppercase tracking-[0.2em] text-[11px] hover:bg-slate-50 transition-all flex items-center gap-3">
                                        <ArrowLeft className="w-4 h-4" /> Back
                                    </button>
                                    <Button
                                        onClick={handleNext} disabled={!isStep2Complete}
                                        className={`h-16 px-12 rounded-2xl font-black uppercase tracking-[0.2em] text-[12px] shadow-2xl transition-all ${isStep2Complete ? "bg-primary text-white hover:scale-105" : "bg-slate-200 text-slate-400 opacity-50"}`}
                                    >
                                        Next Details <ChevronRight className="w-5 h-5 ml-2" />
                                    </Button>
                                </div>
                            </motion.div>
                        )}
                        ...
                        {step === 3 && (
                            <motion.div
                                key="step-3" custom={direction} variants={stepVariants} initial="initial" animate="animate" exit="exit"
                                className="space-y-12"
                            >
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-12">
                                    <div className="space-y-6">
                                        <label className="flex items-center gap-3 text-sm font-black uppercase tracking-[0.2em] text-slate-400">
                                            <Sparkles className="w-4 h-4 text-slate-900" /> Style Preference <span className="text-red-500">*</span>
                                        </label>
                                        <div className="grid grid-cols-2 gap-3">
                                            {STYLE_PREFERENCES.map(s => (
                                                <button
                                                    key={s} type="button" onClick={() => updateData({ stylePreference: s })}
                                    ...
                                    <div className="space-y-6">
                                        <label className="flex items-center gap-3 text-sm font-black uppercase tracking-[0.2em] text-slate-400">
                                            <Building2 className="w-4 h-4 text-slate-900" /> Property Status <span className="text-red-500">*</span>
                                        </label>
                                        <div className="grid grid-cols-2 gap-3">
                                            {PROPERTY_STATUS.map(s => (
                                                <button
                                                    key={s} type="button" onClick={() => updateData({ propertyStatus: s })}
                                ...
                                <div className="p-8 bg-slate-50 rounded-[2rem] border border-slate-100">
                                    <p className="text-[11px] font-black uppercase tracking-[0.3em] text-slate-400 mb-6">Summary Review</p>
                                    <div className="grid grid-cols-2 gap-6">
                                        ...
                                        <div className="flex items-center gap-4 p-4 bg-white rounded-2xl border border-slate-100 shadow-sm">
                                            ...
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-sm font-bold text-slate-900">{data.services[0]}...</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="pt-10 border-t border-slate-100 flex justify-between items-center">
                                    ...
                                    <Button
                                        onClick={handleSubmit} disabled={!isStep3Complete || isSubmitting}
                                        className={`h-16 px-12 rounded-2xl font-black uppercase tracking-[0.2em] text-[12px] shadow-2xl transition-all hover:scale-105 ${isStep3Complete && !isSubmitting ? "bg-primary text-white shadow-primary/20" : "bg-slate-200 text-slate-400 opacity-50"}`}
                                    >
